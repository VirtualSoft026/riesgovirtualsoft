...
# 2. Get shift for May 22, 2026 (Friday)
target_date = datetime.datetime(2026, 5, 22)
gestor_name = 'Juan Jose Diaz Alvarez'
resolved_shift = get_shift_for_date(rows_h, globalScheduleBlocks, gestor_name, target_date)
print(f"\nResolved shift for {gestor_name} on {target_date.strftime('%Y-%m-%d')}: '{resolved_shift}'")

# 3. Load Cronograma Assignments
def get_cronograma_columns_for_today(target_date, shift_text):
    ...
    if js_day == 0:
        ...
        # Weekdays: search both morning and afternoon/evening columns
        return [[1, 2], [4, 5]]

col_groups = get_cronograma_columns_for_today(target_date, resolved_shift)
print(f"Cronograma column groups for shift '{resolved_shift}': {col_groups}")
...
def parse_sheet_range(sheet_name, year=2026):
    ...
    if m:
        ...
        start_date = datetime.datetime(year, months.get(start_m_str, 1), start_day, 0, 0, 0)
        end_date = datetime.datetime(year, months.get(end_m_str, 1), end_day, 23, 59, 59)
    ...
    if m:
        ...
        start_date = datetime.datetime(year, months.get(m_str, 1), start_day, 0, 0, 0)
        end_date = datetime.datetime(year, months.get(m_str, 1), end_day, 23, 59, 59)
        return start_date, end_date
    return None
...
print(f"Sheet selected for {target_date.strftime('%Y-%m-%d')}: '{sheet_name}'")
...
for col_group in col_groups:
    ...
    for r_idx in range(len(rows_c)):
        row = rows_c[r_idx]
        if not row: continue
        if len(row) <= max(t_col, g_col): continue
        ...
        if task_val is not None and str(task_val).strip() != "":
            ...
            if t_str_lower.startswith("set "):
                ...
                if gestor_val is not None and names_match(str(gestor_val).strip(), gestor_name):
                    gestorCronogramaAssignments.append({
                        'set': current_set or 'Otros',
                        'task': t_str
                    })

print(f"Assignments found: {len(gestorCronogramaAssignments)}")
for a in gestorCronogramaAssignments:
    print(f"  Set: '{a['set']}' | Task: '{a['task']}'")

# 4. Filter with Tareas de Riesgo
...
for r_idx in range(1, len(rows_r)):
    r = rows_r[r_idx]
    r_set = r[col_set_idx] if col_set_idx < len(r) else ''
    r_task = r[col_tarea_idx] if col_tarea_idx < len(r) else ''
    
    is_assigned = False
    for assign in gestorCronogramaAssignments:
        if task_names_match(assign['task'], r_task) and set_names_match(assign['set'], r_set):
            is_assigned = True
            break
    if is_assigned:
        filtered_master_rows.append(r)

print(f"Filtered Master Rows: {len(filtered_master_rows)}")
for r in filtered_master_rows:
    print(f"  Set: '{r[col_set_idx]}' | Task: '{r[col_tarea_idx]}'")
