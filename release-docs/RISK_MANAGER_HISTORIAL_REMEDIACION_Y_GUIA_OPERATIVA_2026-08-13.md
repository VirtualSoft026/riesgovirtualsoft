...
| RM-02 | Autorización insuficiente en varias colecciones de datos de la aplicación | Un usuario podía intentar consultar información de otras personas | Reglas de acceso endurecidas por identidad y rol | CORREGIDO EN CANDIDATO LOCAL; VALIDADO EN EMULATOR; R1 PENDIENTE PRODUCTIVO |
| RM-03 | Determinación de rol y aprobación con dependencia parcial del cliente | Riesgo de escalamiento de privilegios | Verificación de rol y aprobación trasladada al servidor | CORREGIDO EN CANDIDATO LOCAL; VALIDADO EN EMULATOR; PENDIENTE PRODUCTIVO |
| RM-04 | Identificador de usuario tomado de una fuente controlable por el cliente | Referencias podían apuntar a otro usuario | Identidad validada contra la sesión autenticada en el servidor | CORREGIDO EN CANDIDATO LOCAL; VALIDADO EN EMULATOR; PENDIENTE PRODUCTIVO |
...
| RM-06 | Lectura global disponible para cualquier usuario autenticado en dos colecciones operativas | Exposición de actividad y reportes de otros usuarios | Lectura global restringida por rol | CORREGIDO EN CANDIDATO LOCAL; VALIDADO EN EMULATOR; PENDIENTE PRODUCTIVO |
...
| RM-08 | Contenido dinámico insertado sin sanitizar en múltiples paneles de la interfaz | Stored XSS | Codificación y sanitización aplicadas en los puntos de inserción afectados | CORREGIDO EN CANDIDATO LOCAL; VALIDADO EN EMULATOR; PENDIENTE PRODUCTIVO |
| RM-09 | El hardening XSS eliminó el formato legítimo de comunicados | Comunicados mostraban etiquetas como texto | Sanitizador con lista de formatos permitidos | CORREGIDO EN CANDIDATO LOCAL; VALIDADO EN EMULATOR; PENDIENTE PRODUCTIVO |
...
| RM-23 | Registros históricos en dos colecciones operativas sin identificador de usuario | Endurecer las reglas ocultaría u obstruiría datos históricos legítimos si no se migran antes | Migración de identificador preparada y ensayada con reversión | PENDIENTE PRODUCTIVO |
...
| RM-30 | Advertencia de índice en una colección operativa | Rendimiento, no autorización | Índice agregado en la definición de reglas | CORREGIDO EN CANDIDATO LOCAL; VALIDADO EN EMULATOR; R1 PENDIENTE PRODUCTIVO |
| RM-31 | Una cuenta pendiente o rechazada podía conservar acceso a datos y a su propio estado de aprobación | Entrada antes de aprobación o autoactivación | Acceso operativo condicionado a la aprobación verificada por el servidor | CORREGIDO EN CANDIDATO LOCAL; VALIDADO EN EMULATOR; R1 PENDIENTE PRODUCTIVO |
| RM-32 | Confirmar lectura de un comunicado permitía escribir sobre el contenido completo | Un usuario aprobado podía intentar alterar título o contenido | Escritura de confirmación de lectura acotada; edición de contenido reservada a Admin | CORREGIDO EN CANDIDATO LOCAL; VALIDADO EN EMULATOR; R1 PENDIENTE PRODUCTIVO |
| RM-33 | Datos dinámicos insertados en atributos y manejadores de eventos de la interfaz | Stored XSS por ruptura de contexto | Codificación aplicada en los puntos de inserción afectados | CORREGIDO EN CANDIDATO LOCAL; VALIDADO CON SMOKE FOCALIZADO; PENDIENTE PRODUCTIVO |
