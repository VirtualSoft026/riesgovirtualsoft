...
Estado productivo: **Fase 1 desplegada en `main` y verificada.** Los documentos de esta carpeta se conservan como registro de planeación y ejecución; cada uno tiene una nota de cierre al inicio con el resultado final. Fase 1.5 (higiene de secretos) tiene su inventario cerrado y una acción de rotación en curso; Fase 1.1 se redefinió a adaptar la interfaz a distintas resoluciones de escritorio/laptop — el acceso móvil se queda bloqueado a propósito por seguridad/política y esta fase no lo toca (ver RM-20). Fase 2 sigue sin iniciar.
